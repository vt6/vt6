...
- the message type name is unknown or its use has not been agreed on by the recipient (see section 4).
...
- the syntax of `want` and `have` messages as described in section 2.1 and 4.
...
Each argument is a module name as accepted by `<module-name>`, or a capability name as accepted by `<message-type>`.
For each argument that is a capability name, the server MUST already have agreed to using the module that defines this capability, or one of the arguments before this argument MUST be that module's name.
...
*Rationale:* A client will typically wait for the `have` reply because the validity of subsequent messages depends on it. A missing `have` message might therefore stall the client forever.
