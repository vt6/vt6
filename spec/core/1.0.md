...
Alternatively, one of the arguments that comes before this argument MUST be a pair of that module's name and a major version such that there exists a specification for that module and major version defining this capability.
...
For `want` arguments that are pairs of module names and major version, the corresponding `have` argument is the corresponding `want` argument, with a dot and a positive integer appended to it.
...
```vt6
(core.sub foo1.bar foo1.baz)
(core.pub foo1.bar hello-world foo1.baz 20)
...
(core.sub foo1.bar foo1.baz)
(core.pub foo1.bar hello-world foo1.baz)
--> invalid; odd number of arguments

(core.sub foo1.bar foo1.baz)
(core.pub foo1.bar hello-world)
--> invalid; missing a property mentioned in the `core.sub` message

(core.sub foo1.bar foo1.baz)
(core.pub foo1.bar hello-world foo1.baz 20 foo1.extra 10)
--> invalid; references a property not mentioned in the `core.sub` message

(core.sub foo1.bar foo1.baz)
(core.pub foo1.baz 20 foo1.bar hello-world)
...
```vt6
# this message...
(core.set foo1.bar value1 foo1.baz value2)
# ...implies this message...
(core.sub foo1.bar foo1.baz)
# ...and may result in a reply like this
(core.pub foo1.bar value1 foo1.baz value3)
```
...
Upon receiving an invalid message, a server or client MAY reply by sending a `core.nope` message. It MUST do so if:
