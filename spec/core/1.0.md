...
Each concrete value of a property is represented as either a `<bytestring>` or a `<list>` (as defined in section 2).
...
### 1.6. Platform integration modules
...
In the context of VT6 messages, a **list** is a sequence of **elements** which are either bytestrings or other lists, such that:
...
Receipt of an invalid message MUST NOT cause any effect that can be observed by the sender, except for error responses that only occur for invalid messages (see section 4.1).
...
Each argument MUST be a bytestring.
...
The message is _not_ invalid if the second argument is a value that is not permissible for the property in question.
