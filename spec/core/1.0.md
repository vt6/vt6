...
A platform SHALL define:

- which operating-system-level objects can be used as basic streams,
- a method which allows the consumer of a basic stream to obtain a new basic stream connected to the same producer as the existing basic stream, and
- a method which allows the producer of a basic stream to obtain a new basic stream connected to the same consumer as the existing basic stream.
...
TODO adjust this example below, and section 6 after deciding what to do with the xxx-msg-bytes-max properties
