...
TODO: define behavior and semantics of multiplexed mode (client identifies as VT6-compatible with magic number `\e[6V`, then switch between message stream and text stream with `\e`; use `\e\e` to represent `\e` inside text/message stream)
...
TODO define rules for module names (`_` prefix for non-official modules, `__` prefix for private modules)
