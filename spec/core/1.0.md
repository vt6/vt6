...
Upon receiving an invalid message, a server or client MUST reply by sending a `core1.nope` message if the message type of the invalid message expected the recipient to answer with some other message.
...
We consider it pointless to try to get any misbehaving process to start behaving correctly by reporting their errors.
