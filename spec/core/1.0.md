...
When a property is said to **accept boolean values**, this means that the set of acceptable values for this property is the set of all atoms whose string value matches the `<integer>` grammar element defined above.
...
If the specification of a module specifies that its module or a capability within that module DEPENDS ON another module or capability, the server MUST NOT agree to using this module or capability if it has already agreed to using the dependency, unless it agrees to using the dependency in the same `have` message.
