...
2. Then, if the specification has been changed in a backwards-incompatible way compared to the previous release (see section 1.6), the major version number is incremented and the minor version number is reset to 0.
...
The first argument MUST be the module name, and the remaining arguments SHALL be the maximal module versions that the client supports.
...
For example, the following `want` message indicates that the client wants to use either major version 1 up to minor version 3 or major version 2 up to minor version 1 of the `example` module.
...
If the server agrees to using the requested module, it MUST reply with a `have` message with two arguments: the module name and the accepted module version.
...
Receipt of the `have` message entitles the client to use all message types, properties and behavior defined in the accepted module version.

The server MUST NOT agree to using a module if it does not support any of the offered module versions.
The server MUST NOT agree to using different versions of the same module across multiple `have` messages on the same server connection.
The server MAY agree to using different versions of the same module on different server connections.

When a `want` message concerns the same module as an earlier `want` message, the server MUST reply consistently.
If the server did not agree to using the module in the earlier `have` message, it MUST NOT agree to using any module versions that were offered by the earlier `want` message.
If the server agreed to using the module in the earlier `have` message, it MUST agree to using the same module version again if (and only if) the agreed module version is offered by the current `want` message.
...
(want foo 2.3)
(have foo 2.3)
--> client can now use core1.0 module and foo2.3 module

(want core 1.2)
(have core 1.0)
(want foo 2.1)
(have foo 2.0)
--> client can only use core1.0 module and foo2.0 module, not the requested higher versions

(want core 1.3 2.1)
(have core 2.1)
--> client can now use core2.1 module, but not core1.x modules
