...
In the context of VT6 messages, a **bytestring** is a sequence of bytes which is:

- it is accepted by `<bytestring>`, and
...
The sequence of bytes between the first colon and the trailing comma is called the **value** of the bytestring.
...
```abnf
list = length "|" *( list / bytestring )
```

A **list** is a sequence of elements which are either bytestrings or other lists, such that:

- it is accepted by `<list>`, and
- the number of elements are the first pipe symbol is equal to the decimal value of the sequence of ASCII digits before the first pipe symbol.
...
We choose netstring-derived lists because they can be implemented very easily, both on the generating and on the parsing side.
They are as flexible and human-readable as s-expressions, JSON, TOML or YAML, but at the same time more compact than either of those.

### 2.1.3. Human-readable representation

Within specifications, the following alternative representation for bytestrings and lists can be used:

- Bytestrings whose value matches the regular expression `^[A-Za-z0-9._-]*$` are represented directly by their value.
- Other bytestrings are represented as C string literals with the same value.
- Lists are represented by a whitespace-separated concatenation of the representations of their elements, enclosed in parentheses.
...
The following snippet shows alternative representations of several example bytestrings and lists.

```
example:      11:hello world,
alternative:  "hello world"

example:      5:hello,
alternative:  hello

example:      2|5:hello,5:world,
alternative:  (hello world)

example:      3|2|3:foo,3:bar,3:abc,3:def,
alternative:  ((foo bar) abc def)

example:      2|2|3:foo,3:bar,2|3:abc,3:def,
alternative:  ((foo bar) (abc def))
...
A **VT6 message** (or just **message**, if the term is not ambiguous) is a nonempty list enclosed in curly brackets whose first element is a bytestring whose value is accepted by `<message-type>`.
...
The message type consists of two **identifiers**, separated by a dot.
The identifier before the dot is the name of the module (see section 3.1) defining that message type.
As an exception, the message types "want", "have" and "nope" do not contain a module name (see sections 4 and 5).

*Rationale:* The curly brackets that enclose messages serve as a sequence point where parsing can be resumed after a parsing error (see section 2.4).
...
Receipt of an invalid message MUST NOT cause any effect that can be observed by the sender, except for error responses that only occur for invalid messages (see section 5.1).
...
Each concrete value of a property is represented as either a `<bytestring>` or a `<list>`.
...
The backwards-compatibility requirement across major versions of `core` does not apply to those parts of the message syntax that can only be used after the initial `want` and `have` messages (namely, nested lists).
...
When a property is said to **accept string values**, this means that the set of acceptable values for this property is the set of all bytestrings whose value is a properly UTF-8-encoded string.
...
When a property is said to **accept integer values**, this means that the set of acceptable values for this property is the set of all bytestrings whose value matches the `<integer>` grammar element defined above.
Analogously, when a property is said to **accept unsigned integer values**, this means that the set of acceptable values for this property is the set of all bytestrings whose value matches the `<unsigned-integer>` grammar element defined above.

In both these cases, the numerical value of each such bytestring is that value which is returned when the `atoll()` function as defined in [the POSIX.1 specification](http://pubs.opengroup.org/onlinepubs/9699919799/functions/atoll.html) is applied to the value of the bytestring.
...
Clients which only want to get the value of a property once are free to ignore subsequent `core.pub` messages.

A `core.sub` message is invalid (within the meaning of section 2.4) if its argument is not the name of a property, or if it is defined by a module to whose usage the server has not agreed.
...
A `core.pub` message is invalid (within the meaning of section 2.4) if its first argument is:
...
A `core.set` message is invalid (within the meaning of section 2.4) if its argument is not the name of a property, or if it is defined by a module to whose usage the server has not agreed.

The message is _not_ invalid if any of its even-numbered arguments is a value that is not permissible for the property in question.
...
That is, a `core.set` message sent from a client to a server implies a `core.sub` message from the same client to the same server for the same property.
